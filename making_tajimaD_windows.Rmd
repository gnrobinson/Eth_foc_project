---
title: "Plotting Tajima D across Genome"
author: "Guy Robinson"
date: "5/6/2021"
...
#  <chr>               <dbl>  <dbl>   <dbl>
#1 Supercontig_2.1        51      1   0.361
#2 Supercontig_2.1      2123      1  -0.672
#3 Supercontig_2.1      2460      1  -1.03 
#4 Supercontig_2.1      2469      1   0.698
#5 Supercontig_2.1      3054      1   0.618
#6 Supercontig_2.1      3180      1  -1.03 
#### This output file was generated from VCFtools --TajimaD ####

#loading total clone data
```{r}
taj.geno <- read_table2("/path/to/file",
     col_names = TRUE, col_types = cols(X4 = col_number()))
colnames(taj.geno)[c(1,2,3,4)] = c("contig", "start_bp", "n_snps", "tajimaD")
```

#making 20Kb windows across the supercontigs
```{r}
mkwindow_avg <- function(infile,midpoint){
  window_size <- subset(infile, start_bp > (midpoint-20000) & start_bp < (midpoint+20000))
  avg_D <- aggregate(tajimaD ~ contig, window_size, FUN=mean)
  avg_D_n <- aggregate(tajimaD ~ contig, window_size, FUN=length)
  avg_D$n <- length(avg_D_n)
  avg_D$midpoint <- midpoint
  colnames(avg_D)[c(1,2,3)] = c("contig", "avg_D","n")
  return(avg_D)
}
```
...
```{r}
N <- max(tal.geno$start_bp)
x <- list()
for (i in seq(10000, N, 10000)) {
  Ps <- mkwindow_avg(taj.geno, i)
...
tajima_plot_df <- rbind(supercontig_2.14_df,
supercontig_2.1_df, 
supercontig_2.6_df, 
supercontig_2.10_df,
supercontig_2.31_df,
...
tajima_plot <- ggplot(tajima_plot_df, aes(x=midpoint_pos, y=avg_D)) +
  theme_minimal() +
  geom_rect(data=NULL,aes(xmin=0,xmax=5961943,ymin=-Inf,ymax=Inf),fill="white") +   geom_rect(data=NULL,aes(xmin=5961943,xmax=11537300,ymin=-Inf,ymax=Inf),fill="lightblue") +  geom_rect(data=NULL,aes(xmin=11537300,xmax=16749062,ymin=-Inf,ymax=Inf),fill="white") +  geom_rect(data=NULL,aes(xmin=16749062,xmax=21662322,ymin=-Inf,ymax=Inf),fill="lightblue") +  geom_rect(data=NULL,aes(xmin=21662322,xmax=26008504,ymin=-Inf,ymax=Inf),fill="white") +  geom_rect(data=NULL,aes(xmin=26008504,xmax=29991914,ymin=-Inf,ymax=Inf),fill="lightblue") +  geom_rect(data=NULL,aes(xmin=29991914,xmax=33295615,ymin=-Inf,ymax=Inf),fill="white") +  geom_rect(data=NULL,aes(xmin=33295615,xmax=36190455,ymin=-Inf,ymax=Inf),fill="lightblue") +  geom_rect(data=NULL,aes(xmin=36190455,xmax=38526589,ymin=-Inf,ymax=Inf),fill="white") +  geom_rect(data=NULL,aes(xmin=38526589,xmax=40758996,ymin=-Inf,ymax=Inf),fill="lightblue") +
